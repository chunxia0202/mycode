...
###################this is for MALC#################################
RESUME_PATH = '/opt/data/private/qianmi/CAN/save_model/MALC_coarse/checkpoint_001.pth.tar'
SAVE_DIR = '/opt/data/private/qianmi/CAN/save_model/MALC_coarse/'
DATA_DIR = '/opt/data/private/CAN/code/Datasets/MALC2012/'
...
class Solver():
    ...
    def validation(self, epoch):
        self.model.eval()
        tbar = tqdm(self.test_loader, desc='\r') # 15
        ...
        with torch.no_grad():
            for ind, sample_batched in enumerate(tbar):
                ...
                for i in range(0, 1, batch_size):
                    ...
                    inf += (end-start)
        print(inf)
        #             flops, params = profile(self.model, (image_3D,skull_3D,image_2D,))
        ...
        #         # 每个个体的256张切片结果总和/256就是每张切片的评价推理时间
        #         # dice and iou evaluation
        #         # labelmap:Tensor:256 256 256
        #         # volume_prediction:Tensor:256 256 256
        ...
        #         visual = True
        #         if visual:
        #             savedir_pred = os.path.join(self.args.save_dir, 'MO-NET')
        ...
        #             # nib.save(volume_prediction, os.path.join(savedir_pred, sample_name[0] + '.nii.gz'))
        #             np.save(os.path.join(savedir_pred, sample_name[0] + '.npy'),volume_prediction)
        #         ####################save output for visualization##################################
        #
